...
var huawei = {
    ...
    article_share: function () {
        if ($("#article-share-this").length > 0) {
            var share_url_list = {
            ...
            var _list = $("#article-share-this li")
            $("#article-share-this").on("click", '.tweet_quote>a', function (e) {
                var $this = $(this);
                var source = $this.parentsUntil("tweet-this", "[data-track]").attr("data-track");
                var title = $this.text() || $this.parent().attr("data-share-title") || document.title;
            ...
            $(document).on("click", ".js_register_form_submit", function (e) {
                ...
                .on("focusout", ".custom-combobox input", function (e) {
                	...
                	if (old_value == new_value)
                		return true;
                	$this.attr("data-old-value", new_value);
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.Stats) {
                    ...
                    $("#hw1_masthead_subinfo_unlogin").attr("style", "display:none;");
...
(function ($) {
    ...
    $.widget("custom.combobox", {
        ...
        _createAutocomplete: function () {
            ...
            var _placeholder = this.element.attr("placeholder") || this.element.attr("data-placeholder");
            this.element.attr("data-placeholder", _placeholder);
        ...
        _removeIfInvalid: function (event, ui) {
            ...
            if (this.element.attr("data-editable") == "1") {
...
function checkVerify() {
    ...
    var verifyCodeUrl = "/en/my-huawei/VerificationCode?validate=" + validateNumber;
...
function jsonpRegisterBack(data) {
    ...
    if (data.status = 1) {
        var msgact = data.msg;
       var thml = "<div class='container'><div class='row returns-prompt'><div class='col-sm-6 center-block'><div class='prompt-contact'><h2>"+msgact+"</h2></div></div></div></div>"
        $(".login-info-cell").html(thml);
    } else {
    	alert("注册失败，请联系我们！");
    }
    $(".js-reg-loading").delay(1000).fadeOut();
